...
Version 200216: Add more Tests and implement "Tagging" mechanism to run selected Tests from the "TestRunner.java" file.
...
public class StepsDefinition extends UtilityCodes {  //By using "extends" we are making the UtilityCodes class the parent of the StesDefinition class, the
		...
		static String wkPlaceId = null;  //global to be used in multiple tests, with value "null" so it can be used in the Hooks.java class.
		...
		// **** 210212 :	
			...
			// the RestAssured API and extract the response in Raw format. 
				...
				respnRaw = requetSpec  
						.when()        
					    	.delete(reqstResource.getResource())  // **** 210215 : Modified to use the Enum class to get the corresponding Resource for the parmAPIName.
						.then()
						   .spec(getCommonResponseSpecifications()) 
						 //  .log().all()
		                .extract().response();														
				...
				System.out.println("**** The HTTP Method indicate "+parmHTTPMethod+" is not valid ***");
			}
		}
		...
		@Then("With created place_id and {string} verify returned name matches {string} provisioned")
		public void with_created_place_id_and_verify_returned_name_matches_provisioned(String parmAPIName, String parmFromInputExpected) throws IOException {
			// After AddPlaceAPI successful completion execute Request "GetPlaceAPI" with the generated "place_id" and verify a match between
			// the 
		    ...
		    System.out.println("*** parmFromInputExpected: "+parmFromInputExpected);
		}

		// **** 210216 : To implement the new Scenario/test for the DELETE functionality.
		@Given("A valid DeletePlace Payload")
		public void a_valid_delete_place_payload() throws IOException {
		    // build the requetSpec for the RestAssured Request using "spec builder" and the serialized JSON payload.  
		    requetSpec =    // this global variable (requetSpec) of type RequestSpecification, contain PART of code for the RestAssure Request, 
					        // the common code using the method getCommonRequestSpecifications() and the serialized body using method data.setTheDeletePlacePojo()
				given()
                   .spec(getCommonRequestSpecifications())  
                                                            
                  // .log().all()
                   .body(data.setTheDeletePlacePojo(wkPlaceId));  //Using the pojo, to serialized JSON body or Payload, from TestDataBuilder method setTheDeletePlacePojo().  
		}
		
		@Given("A valid GetPlaceAPI Request")
		public void a_valid_get_place_api_request() throws IOException {

		    // 1- Prepare execution of the GetPlaceAPI by setting the global variable "requestSpec" to use the GetPlaceAPI Request in the  
			//    @When.. keyword to execute the Request GetPlaceAPI with the "GET" Http Request method.
		    requetSpec =    // this global variable (requetSpec) of type RequestSpecification, contain PART of code for the RestAssure Request, 
            		         // the common code using the method getCommonRequestSpecifications() 
				given()
		           .spec(getCommonRequestSpecifications())  // Using specBuilder for the common code of the Request. the method getCommonRequestSpecifications()
